...
predicted signature. So the "blues are contextually weak" conclusion
SPLITS: it holds for the 6 non-draw blues (removal/heal/burn — the oracle
evaluates them soundly, rate horizon-invariant), but is OVERTURNED for the
2 card-draw blues — the oracle undervalued them, and the fx net playing
them at ~0.30 looks CORRECT, not a mispricing. Caveats: terminal draw-n is
only 31 (rate CI ~[0.10, 0.41], so the crossover is suggestive not clean —
the robust signal is the monotone rise + stable non-draw baseline);
terminal rollout is still random play, under-realizing optimal
card-advantage, so true draw-blue value may be higher. General lesson: the
cheating-MCTS oracle has a known blind spot (card advantage) — don't treat
